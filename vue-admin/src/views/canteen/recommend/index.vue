<template>
    <div class="app-container">
        ...
        <el-table @row-click="clickRow" ref="table" highlight-current-row
                  border v-loading="loading" :data="recommendList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="大卡" align="center" prop="calories"/>
        ...
        <vxe-modal :title="title" v-model="open" width="500px" show-maximize showFooter resize>
            <el-form ref="recommendRef" :model="form" :rules="rules" label-width="80px">
                ...
                <el-form-item label="大卡" prop="calories">
                    <el-input v-model="form.calories" placeholder="请输入大卡"/>
                </el-form-item>
            </el-form>
            <template #footer>
...
import {listRecommend, getRecommend, delRecommend, addRecommend, updateRecommend} from "@/api/canteen/recommend"
...
const data = reactive({
    ...
    rules: {
        ...
        calories: [
            {required: true, message: "大卡不能为空", trigger: "blur"}
        ]
    }
})
...
const handleUpdate = (row) => {
    ...
    getRecommend(_recommendId).then(response => {
...
const handleDelete = (row) => {
    ...
    proxy.$modal.confirm('是否确认删除该项数据？').then(function () {
        return delRecommend(_recommendIds)
    }).then(() => {
        getList()
...
/** 导出按钮操作 */
const handleExport = () => {
    proxy.download('canteen/recommend/export', {
        ...queryParams.value
